...
{
  // double xinit[ninit], convex=1.0, qcent, xcent;			// xjwang
  double *xinit = (double *)malloc(ninit * sizeof(double));	// xjwang
  double convex=1.0, qcent, xcent;							// xjwang
...

{
  ...
  if(q->pl == NULL) {
    ...
    // exit(1);
    a = 0.; // Jun Yan 3/10/2012; fixe me later
  } else if(q->pl->x == q->x) {
